...
suite =
    describe "Analyzer Test"
        [ describe "Simple Path"
            ...
            , test "Occluding" <| \_ -> (analyze simplePath).occluding |> Expect.equal (Set.fromList [])
            , test "Unreachable" <| \_ -> (analyze simplePath).unreachable |> Expect.equal (Set.fromList [])
        ...
        , describe "Isolated Maze"
            ...
            , test "Occluding" <| \_ -> (analyze simplePath).occluding |> Expect.equal (Set.fromList [])
            , test "Unreachable" <| \_ -> (analyze isolatedMaze).unreachable |> Expect.equal
                (Set.fromList
                    [ ( 1, 0, 0 )
                    , ( 2, 2, 1 )
                    ]
        ...
        , describe "Bridge Maze"
            ...
            , test "Occluding" <| \_ -> (analyze simplePath).occluding |> Expect.equal (Set.fromList [])
            , test "Unreachable" <| \_ -> (analyze bridgeMaze).unreachable |> Expect.equal (Set.fromList [ ( 1, 0, 0 ) ])
