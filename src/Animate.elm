...
isAnimatorMoving ( t1, t2, t3 ) state =
    let
        ...
        isSphereMoving target s =
            let
                dv2 = s.velocity.x * s.velocity.x + s.velocity.y * s.velocity.y + s.velocity.z * s.velocity.z
                dp2 = (s.current.x - target.x) * (s.current.x - target.x) + (s.current.y - target.y) * (s.current.y - target.y) + (s.current.z - target.z) * (s.current.z - target.z)
            in
    ...
    let
        ...
        updateSphere curT i target s prevC prevT =
            if curT < toFloat i * staggerDelay then s
                ...
                let
                    targetPos = if i == 0 then target else
                        let dO = { x = target.x - prevT.x, y = target.y - prevT.y, z = target.z - prevT.z } in
                        { x = prevC.x + dO.x, y = prevC.y + dO.y, z = prevC.z + dO.z }
                    diff = { x = targetPos.x - s.current.x, y = targetPos.y - s.current.y, z = targetPos.z - s.current.z }
                    force = { x = diff.x * springK, y = diff.y * springK, z = diff.z * springK }
                    friction = { x = -s.velocity.x * damping, y = -s.velocity.y * damping, z = -s.velocity.z * damping }
                    accel = { x = force.x + friction.x, y = force.y + friction.y, z = force.z + friction.z }
                    newV = { x = s.velocity.x + accel.x * dt, y = s.velocity.y + accel.y * dt, z = s.velocity.z + accel.z * dt }
                    newC = { x = s.current.x + newV.x * dt, y = s.current.y + newV.y * dt, z = s.current.z + newV.z * dt }
        ...
        playerPos ( px, py, pz ) zOffset =
            let
                ...
                fix =
                    ...
                    else (toFloat fix11 * (1 - fx) + toFloat fix21 * fx) * (1 - fy) + (toFloat fix12 * (1 - fx) + toFloat fix22 * fx) * fy
            in
            { x = px * 10, y = py * 10, z = pz * 10 + zOffset + fix }
