...
analyze maze =
    let
        ...
        getNeighbors pos =
            Dict.get pos neighborsMap |> Maybe.withDefault []
        ...
        reachableSet =
            Dict.keys bfsDistances |> Set.fromList

        isReachable =
            Set.member end reachableSet

        shortestPathLength =
            Dict.get end bfsDistances

        -- Components
        c =
            findComponents allCellsList Set.empty 0 getNeighbors
        ...
        riverFactor =
            if (junctions + deadEnds) == 0 then
                0

            else
                toFloat v / toFloat (junctions + deadEnds)
            ...
            case currentLevel of
                [] ->
                    if nextLevel == [] then
                        ...
                        loop nextLevel [] (dist + 1) visited distances
