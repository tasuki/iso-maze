...
drawJoystickDot model =
    case model.joystick of
        ...
        Just j ->
            let
                ( x, y, z ) =
                    Animate.interpolatedPosition model.playerState

                fix =
                    Animate.getFix model.maze ( x, y, z )
                ...
                a =
                    Angle.inRadians model.azimuth

                e =
                    Angle.inRadians model.elevation

                r =
                    { x = -(sin a), y = cos a, z = 0 }

                u =
                    { x = -(cos a * sin e), y = -(sin a * sin e), z = cos e }

                -- 1px = 0.125 units (40px = 5 units = halo radius)
                pixelsToUnits =
                    0.125
